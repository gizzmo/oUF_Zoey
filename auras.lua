...
local defaultAuras = {
    ...
    [175833] = FILTER_ON_PLAYER, -- Draenic Invisibility Potion
    [175817] = FILTER_ON_PLAYER, -- Draenic Living Action Potion
...
if playerClass == 'DRUID' then
    ...
    defaultAuras[155835] = FILTER_ON_PLAYER -- Bristling Fur
    ...
    defaultAuras[108291] = FILTER_ON_PLAYER -- Dream of Cenarius (Balance) -- TODO: check if still exists
    defaultAuras[108292] = FILTER_ON_PLAYER -- Dream of Cenarius (Feral) -- TODO: check if still exists
    defaultAuras[108293] = FILTER_ON_PLAYER -- Dream of Cenarius (Guardian) -- TODO: check if still exists
    defaultAuras[108294] = FILTER_ON_PLAYER -- Dream of Cenarius (Restoration) -- TODO: check if still exists
    defaultAuras[145162] = FILTER_ON_PLAYER -- Dream of Cenarius
...
function ns.UpdateAuraList()
    -- Start with a clean auraList
    wipe(auraList)

    -- Pre filter the aura list
    local PVP = ns.db.PVP
    local playerRoleFilter = roleFilter[GetPlayerRole()]
    for id, v in pairs(ns.defaultAuras) do
        local skip

        -- Aura is something thats not 'always show'
        if bit_band(v, FILTER_ALL) == 0 then

            -- aura is a pvp aura, but pvp isnt enabled
            if (bit_band(v, FILTER_PVP) > 0 and not PVP)

            -- aura is a pve aura, but pvp is enabled.
            or (bit_band(v, FILTER_PVE) > 0 and PVP)

            -- player role doesnt match the filter role.
            or (bit_band(v, FILTER_ROLE_MASK) > 0 and bit_band(v, playerRoleFilter) == 0) then
                skip = true
            end
        end
        if not skip then
    ...
    -- Update all the frames that show auras
